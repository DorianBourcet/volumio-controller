...
def test_reentering_with_identical_frames_does_not_reset_animation():
  # Regression: connecting re-triggers this ~1/s; identical re-entry must not
  # bump the generation, which would restart the frame walk from frame 0.
  frames = ['.  ', '.. ', '...', ' ..', '  .', '']
  display = DisplayState()
  try:
    display.display_persistent_texts(
      texts=frames, duration=0.25, continuous_marquee=False, stop_daemons=False,
    )
    time.sleep(0.05)
    gen_after_start = display._persistent_gen
    for _ in range(5):
      display.display_persistent_texts(
        texts=frames, duration=0.25, continuous_marquee=False, stop_daemons=False,
      )
    assert display._persistent_gen == gen_after_start, 'identical re-entry reset the animation'
  finally:
    display.shutdown()
    loop = display._persistent_loop
    if loop is not None:
