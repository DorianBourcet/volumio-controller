...
- Resilient Volumio connection: bounded exponential backoff on connect, automatic state re-sync on reconnect, I2C bus errors retried once before being dropped
...
└── main_thread.py              - orchestrator with cooperative stop()
    ...
    ├── display_state.py        - display coordinator (I2C lock + safe-write retry)
...
- Volumio OS 4 for Raspberry Pi (I2C must be enabled)
...
### Recommended workflow: Docker (target Python)
...
a `python:3.11-slim-bookworm` container, matching Volumio OS 4 - so a green run here
means green on the device. Only `requirements-dev.txt` is installed; the Adafruit
hardware packages are stubbed by `tests/conftest.py`.
...
For a faster edit/test loop on your host Python, [`uv`](https://docs.astral.sh/uv/) is a fast, isolated package manager. The virtualenv is local to the project (`.venv/`, ignored by git) and can be wiped at any time. Note this runs against your host's Python, not necessarily 3.11 - use Docker (above) to confirm on the target version.
